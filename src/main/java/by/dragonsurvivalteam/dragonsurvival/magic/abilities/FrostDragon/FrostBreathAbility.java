    ...
    public void onDamage(LivingEntity entity) {
        if (!entity.level().isClientSide()) {
            if (entity.isFullyFrozen()) {
                entity.addEffect(new MobEffectInstance(DSEffects.FROSTED, Functions.secondsToTicks(4 * getLevel()), getLevel()));
                entity.setTicksFrozen(entity.getTicksFrozen() + getLevel() * 5);
            } else {
                entity.addEffect(new MobEffectInstance(DSEffects.FROSTED, Functions.secondsToTicks(1.5 * getLevel()), getLevel()));
                entity.setTicksFrozen(entity.getTicksFrozen() + getLevel() * 15);
            }
            entity.addEffect(new MobEffectInstance(DSEffects.BRITTLE, Functions.secondsToTicks(10 * getLevel()), getLevel()));
        }
    }
    ...
    public void onChanneling(Player player, int castDuration) {
        ...
        if(player.level().isClientSide){
            for(int i = 0; i < calculateNumberOfParticles(DragonStateProvider.getData(player).getSize()) / 6; i++){
                ...
                player.level().addAlwaysVisibleParticle(DSParticles.SNOWFLAKE.get(), dx, dy, dz, xSpeed, ySpeed, zSpeed);
            }
            ...
            for(int i = 0; i < calculateNumberOfParticles(DragonStateProvider.getData(player).getSize()) / 12; i++){
                ...
                player.level().addParticle(DSParticles.SNOWFLAKE.get(), dx, dy, dz, xSpeed, ySpeed, zSpeed);
            }
        }
